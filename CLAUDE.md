...
- **`app/api/chat/route.ts`** — POST endpoint. Injects `SYSTEM_PROMPT` + few-shot exemplars + message history into `client.messages.stream()` (Sonnet, max 150 tokens, prompt-cached prefix), returns a `ReadableStream`. Validates request shape (≤50 messages, string content ≤8000 chars) since the deployed route spends the owner's API key.
