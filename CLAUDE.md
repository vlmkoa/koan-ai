...
- **`evals/test_cases.py`** — 29 test cases across 8 archetypes: `confident_mainstream`, `confident_heterodox`, `open_inquiry`, `safety_factual`, `crisis`, `adversarial`, `adversarial_clever`, `factual_widening`. Twelve cases carry `pair_id`/`pair_role` fields linking them into 6 matched mainstream/heterodox topic pairs (astrology, consciousness, religion, alt_medicine, climate, vaccines). The `factual_widening` archetype covers all factual queries — capitals, math, chemistry, time, restaurants — with the expectation of answer-then-widen behavior; the koan stance treats every category as a human convention.
